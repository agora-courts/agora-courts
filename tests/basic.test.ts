...
import { maxDisputeVotes, decimals, courtName, disputeOptions } from './config';
...
describe('agora-court', () => {
    ...
    it('async_court_constructor!', async () => {
        ...
        expect(PublicKey.isOnCurve(cs.repMintAuthority.publicKey)).to.be.true;
    ...
    describe('claim!', () => {
        ...
        describe('claim_all!', () => {
            it('claim_loser!', async () => {
                ...
                // check record
                let recordState = await cs.program.account.voterRecord.fetch(userOne.record.publicKey);
                console.log("claim q: ", recordState.claimQueue);
